...
namespace Aiv.Fast2D.Component {
    public class SheetAnimator : UserComponent {
        ...
        public override Component Clone(GameObject owner) {
            SheetAnimator animatorCloned = new SheetAnimator(owner);
            animatorCloned.spriteRenderer = owner.GetComponent<SpriteRenderer>();
            animatorCloned.clips = new List<SheetClip>();
            for (int i = 0; i < clips.Count; i++) {
                animatorCloned.AddClip(clips[i]);
            }
            animatorCloned.currentClip = currentClip;
            animatorCloned.sliceTime = sliceTime;
            animatorCloned.currentSliceTime = currentSliceTime;
            animatorCloned.currentIndexOfClipFrames = currentIndexOfClipFrames;
            return animatorCloned;
        }

        private SheetAnimator(GameObject owner) : base(owner) {
