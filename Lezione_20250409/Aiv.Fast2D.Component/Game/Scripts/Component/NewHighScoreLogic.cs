...
namespace Aiv.Fast2D.Component {
    public class NewHighScoreLogic : UserComponent {
        ...
        public override void Update() {
            if (!nameInput.ConfirmSelection) return;
            SaveSelection();
        }
        ...
        public void SaveSelection() {
            GameData.leaderboardData.SetScore(GameData.Scores[highScores[currentScoreIndex]], nameInput.Name);
            currentScoreIndex++;
            if (currentScoreIndex < highScores.Count) {
                nameInput.ConfirmSelection = false;
                scoreText.SetText(PaddedScore(GameData.Scores[highScores[currentScoreIndex]]));
                nameInput.ResetName();
            } else {
