...
namespace Aiv.Fast2D.Component {
    ...
    public class GameObject {
        ...
        public void OnCollideEnter (Collision collisionInfo) {
            ICollidable temp;
            foreach (Component component in components) {
                if (!component.Enabled) continue;
                temp = component as ICollidable;
                if (temp == null) continue;
                temp.OnCollideEnter(collisionInfo);
            }
        }

        public void OnCollideExit(Collision collisionInfo) {
            ICollidable temp;
            foreach (Component component in components) {
                if (!component.Enabled) continue;
                temp = component as ICollidable;
                if (temp == null) continue;
                temp.OnCollideExit(collisionInfo);
            }
        }
        ...
        public static GameObject[] FindAll (string tag) {
            return Game.CurrenScene.FindGameObjects(tag);
        }
        #endregion
